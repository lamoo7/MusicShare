<?php
    ...
    if (isset($_POST["submit-comment"])) {
        ...
        $insert_query = "INSERT INTO comments (songname, uploader, songfile, commenter, comment) VALUES ('$songName', '$username', '$songFile', '$commenter', '$comment')";
    ...
    <body>
        ...
        <?php
            ...
            if ($result->num_rows > 0) {
                echo '<select id="order" onchange="location = this.value;">
                        <option value="?order=newest"' . ($order_by == "created_at DESC" ? ' selected' : '') . '>Newest</option>
                        <option value="?order=oldest"' . ($order_by == "created_at ASC" ? ' selected' : '') . '>Oldest</option>
                        <option value="?order=alphabetical"' . ($order_by == "songName ASC" ? ' selected' : '') . '>Alphabetical</option>
                      </select>';
                echo '<div class="feed-wrap">';
                while ($row = $result->fetch_assoc()) {
                  echo '<div class="feed">
                            ...
                            <div class="interaction">
                              <i class="fa fa-heart-o" onclick="updateLikes(' . $row["id"] . ')"><span id="like-count-' . $row["id"] . '">' . $row["likes"] . '</span></i>
            ...
            function updateLikes(id) {
                ...
                xhttp.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        var likeCount = parseInt(document.getElementById("like-count-" + id).innerHTML);
                        document.getElementById("like-count-" + id).innerHTML = likeCount + 1;
                    }
                };
                xhttp.open("POST", "update_likes.php", true);
